  ...
  renderFestivals(
    ...
    setTimeout(() => {
      ...
      items.forEach((item) => {
        ...
        const imageElement = item.firstimage
          ? `<img src="${item.firstimage}" alt="축제 이미지" class="w-full h-[200px] object-cover rounded-[1rem]" />`
          : `<div class="w-full h-[200px] bg-ga-gray300 flex items-center justify-center text-gray-600 text-sm rounded-[1rem]">이미지 없음</div>`;
